...
struct qmi_info {
	...
	struct work_struct rcv_msg_work;
	struct notifier_block svc_nb;
};
...
static struct elem_info qmi_backup_ind_type_ei[] = {
	{
...
static struct elem_info qmi_restore_ind_type_ei[] = {
	{
		.data_type	= QMI_UNSIGNED_1_BYTE,
		.elem_len	= 1,
		.elem_size	= sizeof(u8),
...
static struct elem_info qmi_backup_mem_ready_type_ei[] = {
	{
		.data_type	= QMI_UNSIGNED_1_BYTE,
		.elem_len	= 1,
		.elem_size	= sizeof(u8),
...
static struct elem_info qmi_restore_mem_ready_type_ei[] = {
	{
		.data_type	= QMI_UNSIGNED_1_BYTE,
		.elem_len	= 1,
		.elem_size	= sizeof(u8),
...
static struct elem_info qmi_backup_ind_reg_req_ei[] = {
	{
		.data_type	= QMI_UNSIGNED_1_BYTE,
		.elem_len	= 1,
		.elem_size	= sizeof(u8),
...
static struct elem_info qmi_backup_ind_reg_resp_ei[] = {
	{
...
static struct elem_info qmi_restore_ind_reg_req_ei[] = {
	{
		.data_type	= QMI_UNSIGNED_1_BYTE,
		.elem_len	= 1,
		.elem_size	= sizeof(u8),
...
static struct elem_info qmi_restore_ind_reg_resp_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
		.elem_size	= sizeof(struct qmi_response_type_v01),
...
static struct elem_info qmi_backup_ind_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
		...
		.is_array	= NO_ARRAY,
		.tlv_type	= 0x01,
		.offset		= offsetof(struct qmi_backup_ind, backup_info),
...
static struct elem_info qmi_restore_ind_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
...
static struct elem_info qmi_backup_mem_ready_req_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
...
static struct elem_info qmi_backup_mem_ready_resp_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
		.elem_size	= sizeof(struct qmi_response_type_v01),
...
static struct elem_info qmi_restore_mem_ready_req_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
...
static struct elem_info qmi_restore_mem_ready_resp_ei[] = {
	{
		.data_type	= QMI_STRUCT,
		.elem_len	= 1,
		.elem_size	= sizeof(struct qmi_response_type_v01),
...
{
	int ret;
	int src_vmids[2] = {src, 0};
	int dest_vmids[2] = {dest, 0};
...
static void subsys_qmi_resp_cb(struct qmi_handle *handle, unsigned int msg_id,
...
static int subsys_qmi_send_request(struct subsys_backup *backup_dev,
			...
			*req_ei, void *req_data, int resp_msg_id,
			size_t resp_len, struct elem_info *resp_ei, bool wait)
{
	int ret;
	struct msg_desc req_desc, resp_desc;
	struct qmi_response_type_v01 resp;

	req_desc.msg_id = req_msg_id;
	req_desc.max_msg_len = req_len;
	req_desc.ei_array = req_ei;

	resp_desc.msg_id = resp_msg_id;
	resp_desc.max_msg_len = resp_len;
	resp_desc.ei_array = resp_ei;

	if (wait)
		ret = qmi_send_req_wait(backup_dev->qmi.qmi_svc_handle,
			&req_desc, req_data, sizeof(*req_data), &resp_desc,
			&resp, sizeof(resp), 30000);
	else
		ret = qmi_send_req_nowait(backup_dev->qmi.qmi_svc_handle,
			&req_desc, req_data, sizeof(*req_data), &resp_desc,
			&resp, sizeof(resp), subsys_qmi_resp_cb, backup_dev);

	if (ret < 0) {
		dev_err(backup_dev->dev, "%s: Failed to send request: %d\n",
				__func__, ret);
		return ret;
	}

	if (wait && (resp.result != QMI_RESULT_SUCCESS_V01)) {
		ret = -resp.result;
		return ret;
	}
				...
				enum notif_type type)
{
	...
	ret = subsys_qmi_send_request(backup_dev,
			...
			QMI_BACKUP_MEM_READY_RESP_MSG_ID,
			BACKUP_MEM_READY_RESP_MAX_MSG_LEN,
			qmi_backup_mem_ready_resp_ei, true);
	if (ret) {
				...
				enum notif_type type)
{
	...
	ret = subsys_qmi_send_request(backup_dev,
			...
			QMI_RESTORE_MEM_READY_RESP_MSG_ID,
			RESTORE_MEM_READY_RESP_MAX_MSG_LEN,
			qmi_restore_mem_ready_resp_ei, true);
	if (ret) {
		dev_err(backup_dev->dev, "%s: Failed QMI request: %d",
				__func__, ret);
		goto exit;
...
static void backup_notif_handler(struct subsys_backup *backup_dev,
			struct qmi_backup_ind *backup_ind)
{
	struct qmi_backup_ind_type *ind;
...
static void restore_notif_handler(struct subsys_backup *backup_dev,
			struct qmi_restore_ind *restore_ind)
{
...
static int register_for_backup_restore_notif(struct subsys_backup *backup_dev)
{
	...
	ret = subsys_qmi_send_request(backup_dev,
			...
			QMI_RESTORE_IND_REG_RESP_MSG_ID,
			RESTORE_IND_REG_RESP_MAX_MSG_LEN,
			qmi_restore_ind_reg_resp_ei, true);
	if (ret) {
	...
	ret = subsys_qmi_send_request(backup_dev,
			...
			QMI_BACKUP_IND_REG_RESP_MSG_ID,
			BACKUP_IND_REG_RESP_MAX_MSG_LEN,
			qmi_backup_ind_reg_resp_ei, true);
	if (ret) {
...
static void subsys_ind_cb(struct qmi_handle *handle, unsigned int msg_id,
		void *msg, unsigned int msg_len, void *ind_cb_priv)
{
	struct subsys_backup *backup_dev;
	struct qmi_backup_ind backup_ind;
	struct qmi_restore_ind restore_ind;
	struct msg_desc backup_msg_desc, restore_msg_desc;
	int ret;

	backup_dev = (struct subsys_backup *)ind_cb_priv;

	if (msg_id == QMI_BACKUP_IND_MSG_ID) {
		memset(&backup_ind, 0, sizeof(backup_ind));
		backup_msg_desc.max_msg_len = BACKUP_IND_MAX_MSG_LEN;
		backup_msg_desc.msg_id = QMI_BACKUP_IND_MSG_ID;
		backup_msg_desc.ei_array = qmi_backup_ind_ei;
		ret = qmi_kernel_decode(&backup_msg_desc, &backup_ind, msg,
					msg_len);
		if (ret < 0) {
			dev_err(backup_dev->dev, "%s: Failed decode: %d\n",
					__func__, ret);
			return;
		}

		backup_notif_handler(backup_dev, &backup_ind);

	} else if (msg_id == QMI_RESTORE_IND_MSG_ID) {
		memset(&restore_ind, 0, sizeof(restore_ind));
		restore_msg_desc.max_msg_len = RESTORE_IND_MAX_MSG_LEN;
		restore_msg_desc.msg_id = QMI_RESTORE_IND_MSG_ID;
		restore_msg_desc.ei_array = qmi_restore_ind_ei;
		ret = qmi_kernel_decode(&restore_msg_desc, &restore_ind, msg,
					msg_len);
		if (ret < 0) {
			dev_err(backup_dev->dev, "%s: Failed decode: %d\n",
					__func__, ret);
		...
		restore_notif_handler(backup_dev, &restore_ind);
	} else {
		dev_err(backup_dev->dev, "%s: Ivalid Indication\n", __func__);
	}
}

static void subsys_backup_recv_msg(struct work_struct *work)
{
	int ret;
	struct qmi_info *qmi;
	struct subsys_backup *backup_dev;

	qmi = container_of(work, struct qmi_info, rcv_msg_work);
	backup_dev = container_of(qmi, struct subsys_backup, qmi);

	do {
		dev_info(backup_dev->dev, "Notified about a Receive event\n");
	} while ((ret = qmi_recv_msg(backup_dev->qmi.qmi_svc_handle)) == 0);

	if (ret != -ENOMSG)
		dev_err(backup_dev->dev, "Error receiving message\n");
}

static void subsys_backup_client_notify(struct qmi_handle *handle,
				enum qmi_event_type event, void *notify_priv)
{
	struct subsys_backup *backup_dev;

	backup_dev = (struct subsys_backup *)notify_priv;
...
static void subsys_backup_server_update(struct work_struct *work)
{
	...
	ret = qmi_connect_to_service(backup_dev->qmi.qmi_svc_handle,
	...
	ret = qmi_register_ind_cb(backup_dev->qmi.qmi_svc_handle,
				subsys_ind_cb, backup_dev);
	if (ret < 0) {
		dev_err(backup_dev->dev, "%s: Failed qmi_register_ind_cb:%d\n",
				__func__, ret);
		qmi_handle_destroy(backup_dev->qmi.qmi_svc_handle);
		return;
	}
...
static int subsys_backup_service_event_notify(struct notifier_block *nb,
		unsigned long code, void *_cmd)
{
	struct qmi_info *qmi;
	struct subsys_backup *backup_dev;

	qmi = container_of(nb, struct qmi_info, svc_nb);
	backup_dev = container_of(qmi, struct subsys_backup, qmi);
...
static int subsys_backup_driver_probe(struct platform_device *pdev)
{
	...
	backup_dev->qmi.svc_nb.notifier_call =
					subsys_backup_service_event_notify;
	ret = qmi_svc_event_notifier_register(SUBSYS_BACKUP_SVC_ID,
			SUBSYS_BACKUP_SVC_VERS, SUBSYS_BACKUP_SVC_INS_ID,
			&backup_dev->qmi.svc_nb);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"%s: Failed qmi_svc_event_notifier_register: %d\n",
			__func__, ret);
		goto err;
	}
